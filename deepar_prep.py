...
def write_final_to_json(train, test):
    #TODO: write to S3
    train.coalesce(1).write.mode('overwrite').json('training_input/train/')
    test.coalesce(1).write.mode('overwrite').json('training_input/test/')

write_final_to_json(sorted_list_train, sorted_list_test)
